...
| Modern static assets | `public/favicon.svg`, `public/icon-192.png`, `public/icon-512.png` | Byte-identical Vite starter assets; Vite is MIT, copyright VoidZero Inc. and contributors. The PWA PNGs visibly derive from the same Vite mark but have no recorded generator. | Vite notice is retained, but it does not settle trademark use of the Vite mark as app identity. These three are live — linked from `index.html` and the manifest — so #177 tracks replacing them with original artwork rather than deleting them. The unreferenced `icons.svg` sprite and `hero.png`, which carried social-service marks, have been removed. |
