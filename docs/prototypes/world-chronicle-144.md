...
Status: throwaway prototype. Do not merge this branch as production code.
...
Compare `?chronicle=rail`, `?chronicle=gazette`, and `?chronicle=marginalia` on the same page. The fixed prototype switcher changes only the query string.
