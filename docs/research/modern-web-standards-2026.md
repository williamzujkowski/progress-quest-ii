...
| Vestigial surface | Legacy baseline is clearly separated and intentionally retained. | `src/assets/react.svg`, `src/assets/vite.svg`, and `src/assets/hero.png` have no references. `App.css` contains historical/duplicated styling and many component inline styles remain, increasing visual-regression and CSP maintenance cost. |
