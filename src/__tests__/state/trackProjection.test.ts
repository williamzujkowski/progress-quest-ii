...
import {
  MAX_PROJECTED_SECONDS, projectTrack, retainTrackWindow, type TrackSample,
} from '../../state/trackProjection';
...
): TrackSample => ({ atMs, currentSeconds, maxSeconds, elapsedSeconds });
...
describe('promotion projection', () => {
  it('says nothing until the game clock has advanced far enough to average over', () => {
    ...
    expect(projectTrack([sample(0, 10)])).toBeNull();
    // Ten seconds of play is a rate measured across one long task or none.
    expect(projectTrack([sample(0, 10), sample(10, 20)])).toBeNull();
  });
  ...
  it('projects the remaining track at the observed rate', () => {
    ...
    expect(projectTrack([sample(0, 0, 1000), sample(100, 500, 1000)])).toBe(100);
  });
  ...
  it('projects from the rate rather than assuming a second per second', () => {
    ...
    expect(projectTrack([sample(0, 0, 1000), sample(200, 100, 1000)])).toBe(1800);
  });
  ...
  it('is unmoved by a catch-up drain replaying game time faster than real time', () => {
    ...
    expect(projectTrack(drained)).toBe(projectTrack(live));
    expect(projectTrack(drained)).toBe(1600);
  });
  ...
  it('says nothing across a level-up rather than reporting the reset as a loss', () => {
    ...
    expect(projectTrack([sample(0, 900, 1000), sample(100, 40, 1200)])).toBeNull();
  });
  ...
  it('says nothing when the track has not moved at all', () => {
    ...
    expect(projectTrack([sample(0, 500, 1000), sample(100, 500, 1000)])).toBeNull();
  });
  ...
  it('says nothing while paused, however long the wall clock runs', () => {
    ...
    expect(projectTrack(paused)).toBeNull();
  });
...
describe('the plot track is not a clock either', () => {
  it('advances slower than the game clock, so it needs the same sampled rate', () => {
    // Plot progress advances on every completed task rather than only on kills, which makes it
    // look like a clock. It is not: flooring loses fractions, and it stalls at its ceiling while
    // the act-closing cinematic queues. Measured here rather than assumed, because assuming it
    ...
    let gained = 0;
    let previous = state.character.Plot.currentProgress;
    let previousAct = state.character.Plot.act;

    for (let step = 0; step < 60 * 60 * 20; step += 1) {
      state = advanceGame(state, 50, rng).state;
      const plot = state.character.Plot;
      // Only forward motion inside one act; a reset is not a loss.
      if (plot.act === previousAct && plot.currentProgress >= previous) gained += plot.currentProgress - previous;
      previous = plot.currentProgress;
      previousAct = plot.act;
    }

    const ratio = gained / state.progression.elapsedSeconds;
    expect(ratio).toBeGreaterThan(0);
    expect(ratio).toBeLessThan(0.98);
  });

  it('projects the plot track through the same arithmetic', () => {
    // The projection never cared which track it was handed; this pins that.
    const plotSamples = [
      { atMs: 0, currentSeconds: 0, maxSeconds: 3_600, elapsedSeconds: 0 },
      { atMs: 200_000, currentSeconds: 900, maxSeconds: 3_600, elapsedSeconds: 1_000 },
    ];
    // 900 progress across 1000 elapsed seconds; 2700 remain, so 3000 seconds of play.
    expect(projectTrack(plotSamples)).toBe(3_000);
  });
});
